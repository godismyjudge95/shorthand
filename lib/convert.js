...
Expected input: set of CSS rules written with proper format and order, separated by linebreaks
...
var rule = {
    ...
    condense: function (rules) {
        ...
        rules.forEach(function(rule) {
            values += rule.split(":")[1].replace(/(^\s+|\s+$)/, "") + ' ';
        })
        
        var result = {};
        return result[rules[0].split(":")[0].split("-")[0]] = values.substr(0, values.length -1);
